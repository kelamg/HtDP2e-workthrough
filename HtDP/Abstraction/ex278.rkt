...
(define C-TEXT     (text C-MSG TEXT-SIZE TEXT-COLOR))
(define E-MSG      "worm went cannibal mode : ")
(define E-TEXT     (text E-MSG TEXT-SIZE TEXT-COLOR))
...
(define (control-worm g ke)
  (cond
    [(and (key=? ke "up")
     ...
     (make-game
      (make-worm
       (worm-head (game-worm g)) (worm-segs (game-worm g)) "u")
      (game-food g))]
    [(and (key=? ke "down")
     ...
     (make-game
      (make-worm
       (worm-head (game-worm g)) (worm-segs (game-worm g)) "d")
      (game-food g))]
    [(and (key=? ke "left")
     ...
     (make-game
      (make-worm
       (worm-head (game-worm g)) (worm-segs (game-worm g)) "l")
      (game-food g))]
    [(and (key=? ke "right")
     ...
     (make-game
      (make-worm
       (worm-head (game-worm g)) (worm-segs (game-worm g)) "r")
      (game-food g))]
    [else g]))
...
;; produces an image of the final score
(check-expect
 (render-score '())
 (text "1" TEXT-SIZE TEXT-COLOR))
(check-expect
 (render-score (list (make-posn 30 55)))
 (text "2" TEXT-SIZE TEXT-COLOR))
(check-expect
 (render-score (worm-segs (game-worm G3)))
 (text "7" TEXT-SIZE TEXT-COLOR))

(define (render-score segs)
  (text (number->string (+ (length segs) 1))
        TEXT-SIZE
        TEXT-COLOR))

;; Game Image -> Image
;; render final score text image
(check-expect
 (render-score-text
  C-TEXT
  (make-game (make-worm (make-posn WIDTH 50) '() "r") F1))
...
(define (render-score-text text gs)
  (place-image
   (beside text
          (render-score (worm-segs (game-worm gs))))
  TEXT-X TEXT-Y
  (render-worm (game-worm gs))))
...
(check-expect
 (render-game-over G3)
 (render-score-text E-TEXT G3))

(define (render-game-over g)
  (if (cannibal-worm? g)
      (render-score-text E-TEXT g)
      (render-score-text C-TEXT g)))
