  ...
  (cond
    ...
    [else
     (+ (second (find-association
                 "time" (first ll) (list 0 0)))
        (total-time/list (rest ll)))]))

;; when run on itunes.xml, total-time/list produces 0 compared
;; to the actual total time produces by total-time.
;; This happens because whereas total-time takes a struct which
;; abstracts away the need to know the name of the time field
;; on the xml file. total-time/list however, hardcodes a field
;; name "time" in the function body; the field name on the xml
;; file is "Total Time", thus the function produces 0.
